...
func TestHarnessLogsAndSnapshots(t *testing.T) {
	...
	require.NoError(t, h.Step(context.Background(), Op{Kind: OpPut, K: []byte("a"), V: []byte("b")}))
	require.Equal(t, uint64(1), h.Seq)

	require.NoError(t, h.Step(context.Background(), Op{Kind: OpSnap}))
...
func TestHarnessRunChecksInvariants(t *testing.T) {
	...
	require.NoError(t, h.Run(context.Background(), cfg, 50))
}
