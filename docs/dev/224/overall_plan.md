...
3. **Parse footer** when loading sparse index, verifying magic number and reading index block.
...
func loadSparseIndex(fs *FileSystem) (SparseIndex, error) {
    ...
    f, err := readFooter(fs, end-footerSize)
    if err != nil { return nil, err }
    reader := newOffsetReader(fs, int64(f.indexOffset))
